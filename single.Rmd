...
```{r}
c <- c(1,2,3,4)
g <- read.graph("test0411/6-1.graphml", "graphml")
...
# tic('brute force')
# for(i in 1:length(parts)) {
#   result <- brute_force(parts[[i]], c)
#   V(graphB)[V(result[[1]])$name]$color <- V(result[[1]])$color
#   sumB <- sumB + result[[2]]
# }
# toc()
# 
# plot(graphB, layout=layout_as_bipartite, palette=diverging_pal(4), main=paste0('brute force:', sumB), vertex.size=40, vertex.label.cex=1, vertex.label = paste0(V(graphB)$name, '/', V(graphB)$weight, '/', V(graphB)$color))
# ```
