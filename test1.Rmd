...
for (i in 1:length(density)) {
  for(j in 1:1) {
    ...
    for(k in 1:length(parts)) {
      ...
      # Option 1 <- split brute force with graph decompose
    ...
    df <- rbind(df, data.frame(list(group = i, counter = j, density = density[i], approximated = sumA, optimal = sumB, ratio = sumA / sumB)))
    
    write_graph(graph, paste0(path, i ,'-', j, '.graphml'), "graphml")
  }
}
toc()
