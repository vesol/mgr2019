...
# threePseudoColoring <- three_pseudocoloring(g, colors)
# V1 <- threePseudoColoring[[1]]
# a <- sum(V(g)[V1]$weight)
# b <- sum(V(g)[type==TRUE]$weight)
# c <- sum(V(g)[type==FALSE]$weight)
# 
# a == b
# a == c
# print(sum(V(g)[V1]$weight) == sum(V(g)[type==TRUE]$weight))
# print(sum(V(g)[V1]$weight))
# print(sum(V(g)[type==FALSE]$weight))
# print(sum(V(g)[type==TRUE]$weight))
# Ssum <- sum_coloring(S, c)

plot(S, layout=layout_as_bipartite, palette=diverging_pal(4), vertex.size=40, vertex.label.cex=1, main=paste('27/26-approximate - cost: ', Ssum, ' (optimal:', optimal, ')'), vertex.label = paste(V(S)$name, '/', V(S)$weight, '/', V(S)$color, sep=''))
```
# Brute force
```{r}
