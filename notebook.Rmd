...
S <- result[[1]]
Ssum <- result[[2]]
optimal <- is_optimal(g, c)
toc()

plot(S, layout=layout_as_bipartite, palette=diverging_pal(length(unique(V(S)$color))), vertex.size=40, vertex.label.cex=1, main=paste('27/26-approximate - cost: ', Ssum, ' (optimal:', optimal, ')'), vertex.label = paste(V(S)$name, '/', V(S)$weight, '/', V(S)$color, sep=''))
```
# Brute force
```{r}
