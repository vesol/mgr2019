...
saveRDS(df, file = 'test1/data.rds')
```
# Playing with data frame
```{r}
df <- readRDS(file = 'test1/data.rds')
...
for (i in 1:10) {
  ...
  g <- read.graph(paste0('test1/', group, '/', counter,'.graphml'), "graphml")
  ...
  write_graph(g, paste0('test1/fixed/', group ,'-', counter, '.graphml'), "graphml")
}

toc()
saveRDS(fixed, file = 'test1/fixed.rds')

```
```{r}
fixed
```
# Compare algorithms for single graph
```{r}
source("src/3-pseudocoloring.R")
source("src/2-coloring.R")
source("src/is_colored_properly.R")
...
g <- read.graph("test1/1/25.graphml", "graphml")
